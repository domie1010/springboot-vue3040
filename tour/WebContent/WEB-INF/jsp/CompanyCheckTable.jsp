				...
				<tbody>
					...
					<c:forEach items="${pager.datas }" var="user" varStatus="vg">
						<tr>
							...
							<td><c:out value="${user['recommendId'] }"></c:out></td>
							<td><c:out value="${user['address'] }"></c:out></td>
							<td><a href="user/success?id=${user['id'] }"
								class="tablelink">通过</a>
								<a href="javascript:;"
								onclick="deletecheck(${user['id']})"
								class="tablelink">不通过</a></td>
						</tr>
					</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="user/queryCompanyCheck?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="user/queryCompanyCheck?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="user/queryCompanyCheck?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="user/queryCompanyCheck?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="user/queryCompanyCheck?pageNow=${count }">${count}</a></li>
