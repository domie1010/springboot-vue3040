...
<script type=text/javascript src="js/js/foreground/usert.js"></script>
...
<div class="shopquery">
		<div class="fang" style="width:120px;height:120px">
			...
			<a href="act/companyAct?id=${act['id']}" target="_self" ><img src="upload/upload/${act['url'] }" style="width:120px;height:120px" border="0" /></a>
			</div>
		</div>
		<div class="prop">
		活动名称：<span class="hb"></span><span class="price">${act['name'] }</span><br />
		</div>
		<div class="memo" >
		旅行社:${act['userInfo']['userName'] }
		</div>
</div>
</c:forEach>
</div>
...
<div id="showpages" >
	  ...
	  <a href="act/informationLoadCompany?pageNow=${pager.pageNow }">上一页</a>
	  </c:if>
	   <c:if test="${!pagele}">
	  <a href="act/informationLoadCompany?pageNow=${pager.pageNow -1}">上一页</a>
	  </c:if>
	  </li>
	  <li class="pbutton">
	  <c:if test="${pager.pageNow ge pager.totalPages }" var="pagege">
	  <a href="act/informationLoadCompany?pageNow=${pager.pageNow }">
	  下一页
	  </a>
	  </c:if>
	  <c:if test="${!pagege }">
	  <a href="act/informationLoadCompany?pageNow=${pager.pageNow+1 }">
	  下一页
	  </a>
	  </c:if>
	  </li></ul></div>
