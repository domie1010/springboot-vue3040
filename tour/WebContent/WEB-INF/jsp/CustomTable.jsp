				...
				<tbody>
					...
					<c:forEach items="${pager.datas }" var="user" varStatus="vg">
						<tr>
							...
							<c:if test="${user['real']['status'] eq 1}">
								<td><c:out value="${user['real']['name'] }"></c:out></td>
								<td><c:out value="${user['real']['age'] }"></c:out></td>
								<td>
									<c:if test="${user['real']['sex'] eq 1}">
										<c:out value="男" />
									</c:if>
									<c:if test="${user['real']['sex'] eq 2}">
										<c:out value="女" />
									</c:if>
								</td>
								<td><c:out value="${user['real']['phone'] }"></c:out></td>
							</c:if>
							<c:if test="${user['real']['status'] eq 0}">
								<td colspan="4"><c:out value="暂未申请认证"></c:out></td>
							</c:if>
