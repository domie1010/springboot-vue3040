...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/actAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("act/informationLoadOne?id="+id);
	        }
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${actList.datas }" var="act" varStatus="vg">
						<tr>
							<td><c:out value="${act['name']}"></c:out></td>
							<td><c:out value="${act['userInfo']['userName']}"></c:out></td>
							<td><c:out value="${act['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${act['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${act['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="act/informationLoad?pageNow=${pager.pageNow }&type=${act['type']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="act/informationLoad?pageNow=${pager.pageNow-1 }&type=${act['type']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="act/informationLoad?pageNow=${vs.count }&type=${act['type']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="act/informationLoad?pageNow=${count }&type=${act['type']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="act/informationLoad?pageNow=${count }&type=${act['type']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="act/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
